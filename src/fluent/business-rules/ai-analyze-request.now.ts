 ...
 * Triggers after a new FlowDesk Request is inserted or updated.
 *
 * IMPORTANT: We use an inline `script` string (not a module reference)
 * because module-scoped Business Rules run in a sealed ES Module sandbox
 * where sn_ws.RESTMessageV2 and other runtime globals are NOT available.
 * Inline scripts execute in ServiceNow's classic Rhino engine where
 * all server-side APIs (gs, sn_ws, GlideRecord, etc.) work normally.
 */
BusinessRule({
...
(function executeRule(current, previous) {
    try {
        ...
        // Boolean fields return '1' (true) or '0' (false) in ServiceNow
        ...
        // Read API key from System Property
        ...
        restMessage.setEndpoint('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-05-20:generateContent?key=' + apiKey);
